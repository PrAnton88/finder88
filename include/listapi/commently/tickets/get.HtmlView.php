	...
	if(!$chunk){ 
		...
		if($comment != ''){
			...
			echo $chunk->process(array(
				'id' => $nRecord, /* номер заявки или номер ресурса */
			...
			(function(){
				let dataMessage ={}; 
				dataMessage.nrequest = $nRecord;
				
				dataMessage.subject = 'Оповещение ";
				
				if($isPrivate){
					
					echo "ответственного';";
					echo "dataMessage.note = 'оповещаем ответственного на заявку. ";
					
					
					if($hidden == 0){
						echo "Один из родителлей комментария - скрытый";
					}/* else этот комментарий скрытый */
					
					echo "';";
					
				}elseif($message['applicant']==$user['uid']){
					/* от заявителя */
					
					/* то оповещаем ответственного на заявку (сообщение от инициатора) */
					echo "ответственного';";
					echo "dataMessage.note = 'оповещаем ответственного на заявку';";
					
				}else{
					/* от ответственного или от любого из админов и коммент не скрыт */
					/* оповещаем инициатора заявки */
					
					$applicantFio = $message['ass'];
					
					echo "инициатора заявки';";
					echo "dataMessage.description = 'Здравствуйте $applicantFio!';";
					echo "dataMessage.note = 'оповещаем инициатора заявки';";
				}
				
				echo"
dataMessage.theme = 'Поступил новый комментарий на заявку';
oBaseAPI.message.email.";

			if($message['applicant']==$user['uid']){
				/* то оповещаем ответственного на заявку */
				echo "sendToAssdTickets";
			}elseif($admin != 0){
				/* то оповещаем инициатора заявки */
				echo "sendToApplicantTickets";
			}
			echo "(dataMessage,function(){ customMess('Сообщение отправлено') });
			})();";
