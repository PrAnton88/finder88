<?php
	...
	function getQueryToCheckBusyDevices($type=''){
		if($type != ''){ $type .= ' AND'; }
		return 'SELECT n.id,n.name, n.description FROM bronedevicename as n
	WHERE '.$type.' n.hidden=0 AND ((n.id IN (SELECT e.id FROM bronedevicedate as b, bronedevicename as e WHERE ((b.iddevice=e.id) AND (b.free=0 OR b.free=2) AND (e.hidden=0))	)) ) AND count>0 
	ORDER BY n.name';
