...
try{
	...
	if($nrequest === false){
		...
		$dtime = false;
		if(is_array($existmess) && (count($existmess) > 0)){
			
			$existmess = $existmess[count($existmess)-1];
		...
		if (!($existmess and (/*если не дубирует созданные*/($dtime !== false) && (($dtime->mktime()) > (time() - 600)) ))){
