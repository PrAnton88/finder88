...
try{
	...
	/* use exam
		...
		dataReqNext({file:urlServerSide+'helpdesk/get.Html.infoCountComments.item.php',type:'text',
	...
	*/
	...
	$ccomment = $commentsHand->getCount($nrequest,$admin);
	
	$message = array('ccomment' => $ccomment);
	
	if($ccomment > 0){
		$message['lastcomment'] = $commentsHand->getLastComment($nrequest,($admin==0?false:true));
	}
	...
	return $smarty->display('../chunks/components/tmp.row.item.helpdesk.countComments.tpl');
