...
function content_610ce1079b60c6_62414138 (Smarty_Internal_Template $_smarty_tpl) {
echo '<script'; ?>
>
	...
	oFormtoBroneDevices = (function(){
		...
		function checkLoadDevices(item,funcLoaded){
			if(item.listDevices == 0){
				...
				*/
				if(!oDataDriver){
					throw new Error('не установлен DataDriver');
				}
				
				oDataDriver.getDataTypeOfDevices(item.id,function(resplistDataDevices){

					item.draw(resplistDataDevices);
					funcLoaded(item);
				});
		...
		let oDevices = {
			/* список всех типов - не изменяемая величина */
			listRecords:listEveryTypeRecords.get(),
			
			getChecked:function(){
				
				let checkedList = []; 
				/* listEveryTypeRecords.get()*/this.listRecords.map(item => {
					if((item.listDevices.length > 0 /* когда ранее было загружено содержимое типа */) && (item.checkedDevices.get().length > 0)){
						checkedList.push({
			...
			checkOut:function(){
				/*
				let records = listEveryTypeRecords.get();
				*/let records = this.listRecords;
				let devices = null;
				/*
				console.log(listEveryTypeRecords);
				*/
				records.map(item => {
					devices = item.checkedDevices.get();
					if(devices.length > 0){
						devices.map(device => {
							device.check();
						});
					...
					/*
					console.log(item);
					listEveryTypeRecords.toggle(item);
					*/
				});
				
			
			},
			check:function(listLoadDevices){
				
				listLoadDevices.map(loadDevice => {
				
					/* loadDevice.id это id типа */
					let types = listEveryTypeRecords.get(loadDevice.id);
					types.map(type => {
						
						/* откроем устройства и загрузим их */
		...
		return {
			htmlForm:htmlForm,
			sectionHandComplete:fget('sectionHandComplete'),
			close:function(){
			...
			clearAll:function(){
				...
				htmlForm.querySelector('textarea').value = '';
				oDevices.checkOut();
			},
			setDataDriver:function(dataDriver){
			
				oDataDriver = dataDriver;
				/*
				oDataDriver = oDataDriverToFormBroneDevices; */
			},
			data:{
				set:function(listLoadDevices,note,idRecord/*editRecord*/){
				
					/* например для загрузки на редактирование
					oDevices.check([
					...
					*/
					...
					oDataDriver.editRecordId = idRecord;
					/* например для загрузки на редактирование */
				
					oDevices.check(listLoadDevices);
					
					if(!note){ note = ''; } note += '';
					htmlForm.querySelector('textarea').value = note;
				},
				get:function(){
					return {
						checkedDevicesTypes:oDevices.getChecked(),
						note:htmlForm.querySelector('textarea').value
					}
